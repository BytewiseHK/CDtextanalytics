...
def fetch_page(url):
    ...
    Handles encoding issues to ensure proper display of Chinese characters.
    """
    try:
...
def analyze_url_structure(url, soup, output_folder):
    """
    Analyzes the structure of a webpage and writes a report to the output folder.
    """
    report_file = generate_unique_filename("URL_report", "txt")
    ...
    try:
        with open(report_path, "w", encoding="utf-8") as f:
            ...
            # Write structure of the page (div classes and IDs)
...
def extract_articles(soup, base_url):
    """
    Extracts all articles (links with visible text) from the webpage.
    """
    articles = []
    for link in soup.find_all("a", href=True):
...
def extract_content(soup):
    """
    Extracts the title, publication date, source, and main content from a webpage.
    """
    try:
...
def save_content_to_file(content, output_folder):
    """
    Saves the extracted content (title, date, source, content) to a text file.
    """
    if not content or content["content"] == "No content found":
...
def display_and_choose_article(articles):
    """
    Displays a list of extracted articles and allows the user to select one to visit.
    """
    print("\nExtracted Articles:")
...
def main():
    """
    Main function to fetch and process a webpage, allowing recursive navigation.
    """
    # Base output folder
