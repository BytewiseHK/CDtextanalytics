...
def scrape_recursive(url, visited, base_url, depth=3, max_urls=50):
    if depth == 0 or url in visited or len(visited) >= max_urls:
        return []
    visited.add(url)

    # Fetch and parse the page
    print(f"Visiting: {url} (Depth: {depth}, URLs Visited: {len(visited)})")
    soup = fetch_page(url)
    if not soup:
        return []
    ...
    for article in articles:
        if article["url"] not in visited:
            ...
            time.sleep(1)  # Rate limiting: Add a delay between requests

    # Recursively visit links on this page
    for article in articles:
        print(f"Processing article: {article['title']} (URL: {article['url']})")
        # Ask user if they want to continue after every 10 URLs visited
        if len(visited) % 10 == 0:
            print(f"\n--- Progress Report ---")
            print(f"URLs Visited: {len(visited)}, Articles Downloaded: {len(results)}")
            print(f"Current Depth: {depth}")
            user_input = input("Do you want to continue? (y/n/save/exit): ").strip().lower()
            if user_input == "n":
                print("Stopping crawl as requested by the user.")
                return results
        ...
        results.extend(scrape_recursive(article["url"], visited, base_url, depth - 1, max_urls))
...
def main():
    ...
    visited_urls = set()
    results = scrape_recursive(start_url, visited_urls, start_url, depth=3, max_urls=50)
